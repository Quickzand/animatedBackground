...
class Cell {
    ...
    additionalInset = 0; // Added property for snapShrink
    ...
    draw() {
        ...
        gridCTX.fillStyle = "rgba(18, 18, 18, " + drawOpacity + ")";

        // Drop shadow
        gridCTX.shadowOffsetX = 0;
        gridCTX.shadowOffsetY = 30;
        gridCTX.shadowBlur = 20;
        cellGap = 0;
        gridCTX.shadowColor = 'rgba(255, 255, 255, 0)';
        cellGap = 0;
        this.additionalInset = 0;
        // gridCTX.fillRect(
        //     this.x + (cellGap / 2) + this.additionalInset, // Adjust for additionalInset
        //     this.y + (cellGap / 2) + this.additionalInset, // Adjust for additionalInset
        //     this.currentSize - 2 * this.additionalInset, // Adjust for additionalInset
        //     this.currentSize - 2 * this.additionalInset  // Adjust for additionalInset
        // );
        ...
        // Put shadow around cell


        // Reset shadow properties to ensure other elements don't get affected.
        gridCTX.shadowOffsetX = 0;
...
cellSize = Math.min($(window).width() / colCount, $(window).height() / rowCount);
...
function createCells(colCount, rowCount, skipIntro) {
    ...
    for (var i = 0; i < rowCount; i++) {
        ...
        for (var j = 0; j < colCount; j++) {
            cells[i].push(
                new Cell(startX + j * (cellSize + cellGap), startY + i * (cellSize + cellGap), cellCount * 10, skipIntro)
            );
            cellCount++;
        }
    }
...
function animateRaindrop() {
    console.log("Animating a raindrop")
    var rainDrop = [];
    ...
    function animateCircle() {
        for (var i = 0; i < rowCount; i++) {
            for (var j = 0; j < colCount; j++) {
                ...
                var distance = Math.sqrt(Math.pow(j - centerX, 2) + Math.pow(i - centerY, 2));
        ...
        if (radius < maxRadius + 5) {
            setTimeout(animateCircle, 150);  // adjust the time as needed
        }
    }
